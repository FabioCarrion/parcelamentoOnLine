...
public class SolicitacaoRepository {
        ...
        public static void findAll () throws SQLException, ClassNotFoundException {
            ...
            PreparedStatement statement = conn.prepareStatement("SELECT * FROM telegram.solicitacao ");
            ResultSet resultSet = null;
            resultSet = statement.executeQuery();

            System.out.println("---- BUSCA POR ID INI ----");
            while (resultSet.next()) {
                String index = resultSet.getString(3);
        ...
        public static void findById (Integer id) throws SQLException, ClassNotFoundException {
            Connection conn = ConnectionManager.getConnection();
            PreparedStatement statement3 = conn.prepareStatement("SELECT * FROM telegram.solicitacao WHERE idcontato = ?");
